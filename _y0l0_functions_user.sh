...
_ssl_pem_valid_in_future_seconds() { openssl x509 -checkend $1 -noout -in "$2" ; } ; #86400 seconds for one day
_ssl_pem_enddate() { printf '%s: %s\n' "$(date --date="$(openssl x509 -enddate -noout -in "$1"|cut -d= -f 2)" --iso-8601)" "$1"  ; } ; 
_ssl_host_valid_in_future_seconds() { echo | openssl s_client -connect "$2" 2>/dev/null | openssl x509 -checkend $1 -noout  ; } ; #86400 seconds for one day
_ssl_host_enddate() { printf '%s: %s\n' "$(date --date="$( echo | openssl s_client -connect "$2" 2>/dev/null |openssl x509 -enddate -noout |cut -d= -f 2)" --iso-8601)" "$1"  ; } ; 
_ssl_smtp_valid_in_future_seconds() { echo |openssl s_client -connect "$2" -starttls smtp 2>/dev/null | openssl x509 -checkend $1 ; } ; #86400 seconds for one day
_ssl_smtp_enddate() { printf '%s: %s\n' "$(date --date="$( echo | openssl s_client -connect "$2" -starttls smtp 2>/dev/null | openssl x509 -enddate -noout |cut -d= -f 2)" --iso-8601)" "$1"  ; } ; 
